                    ...
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-6 py-4">
                            ...
                            <div class="mt-1 text-xs text-gray-600">
                                @foreach($sale->saleItems->take(5) as $item)
                                    {{ $item->part->name ?? 'N/A' }}{{ $item->quantity > 1 ? ' × ' . $item->quantity : '' }}@if(!$loop->last), @endif
                                @endforeach
                                @if($sale->saleItems->count() > 5)
                                    <span class="text-gray-500">+{{ $sale->saleItems->count() - 5 }} more</span>
                                @endif
                            </div>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div class="flex items-center justify-end gap-3">
                                <a href="{{ route('sales.edit', $sale) }}" class="text-amber-600 hover:text-amber-900 flex items-center gap-1" title="Edit sale">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                    </svg>
                                    Edit
                                </a>
                                <a href="{{ route('sales.show', $sale) }}" class="text-blue-600 hover:text-blue-900 flex items-center gap-1">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
