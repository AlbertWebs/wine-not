...
@section('title', 'Edit Website Product')

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="mb-6">
        <a href="{{ route('website.products.index') }}" class="text-blue-600 hover:text-blue-800 flex items-center gap-2 mb-4">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path>
            </svg>
            Back to Products
        </a>
        <h1 class="text-3xl font-bold text-gray-900">Edit Website Product</h1>
        <p class="text-gray-600 mt-1">{{ $product->name }}</p>
    </div>

    <div class="bg-white rounded-lg shadow-md p-8">
        <form method="POST" action="{{ route('website.products.update', $product) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6 pb-6 border-b">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <p class="text-sm text-gray-900">{{ $product->part_number }}</p>
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">SKU</label>
            ...
            <div class="mb-6">
                ...
                <div class="mb-4">
                    ...
                    <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" class="h-48 w-48 object-cover rounded-lg border border-gray-300" id="current-image">
                </div>
                @endif
            ...
            <div class="flex justify-end gap-3">
                <a href="{{ route('website.products.index') }}" class="px-6 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 transition">
                    Cancel
                </a>
